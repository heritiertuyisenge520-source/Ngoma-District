...
const FillFormView: React.FC<FillFormViewProps> = ({ entries, onAddEntry, onClear, initialEntry, onCancelEdit, userEmail, userName, userRole, userType, userUnit, assignedIndicators = [] }) => {
  ...
  const submitData = async (action: 'edit' | 'increment', existingSubmissionId?: string) => {
    ...
    const payload = {
      ...
      subValues: Object.keys(numericSubValues).length > 0 ? numericSubValues : undefined,
      comments: comments,
    ...
    try {
      ...
      setSuccessMessage(`Failed to submit data: ${error}`);
      setShowSuccessModal(true);
    }
  };
  ...
  return (
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 w-full">
      ...
      {submissionPeriod && !isLoadingSubmissionPeriod && !submissionPeriodError && (
        <div className="bg-red-600 text-white rounded-xl px-4 py-2.5 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <p className="text-xs font-semibold">{submissionPeriod.description}</p>
              <p className="text-[10px] opacity-90">
                {formatDate(submissionPeriod.startDate)} - {formatDate(submissionPeriod.endDate)}
              </p>
            </div>
          </div>
          <p className="text-xs font-bold">{countdown}</p>
        </div>
      )}
